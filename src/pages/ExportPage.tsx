...
export default function ExportPage() {
  ...
  const filterPurchasesByDate = useCallback((purchases: FishPurchase[], dateFilter: DateFilter): FishPurchase[] => {
  ...
  const groupPurchasesIntoSessions = useCallback((purchases: FishPurchase[]): PurchaseEntrySession[] => {
    ...
    sessions.forEach((session, index) => {
      ...
      const worksheet = workbook.addWorksheet(worksheetName, {
        pageSetup: {
          ...
          printArea: 'A4:H50' // Print area for A4 - start from the table headers
        }
      });
      ...
      worksheet.columns = [
        ...
        { header: 'NET KG PER MC', key: 'netKgPerMc', width: 10 },
        { header: 'QTY MC', key: 'qtyMc', width: 7 },
        { header: 'QTY KGS', key: 'qtyKgs', width: 7 },
        { header: 'PRICE (KG)', key: 'pricePerKg', width: 8 },
        { header: 'TOTAL USD AMOUNT', key: 'totalAmount', width: 10 }
      ];

      // Add title outside the table - centered
      ...
      headerRow.height = 16; // Smaller row height to match screenshot
      
      // Style the header row
      headerRow.eachCell((cell) => {
        cell.font = { bold: true, size: 10 }; // Larger font size for better readability
        cell.border = {
          top: { style: 'thin' },
          left: { style: 'thin' },
          bottom: { style: 'thin' },
      ...
      Object.entries(fishTypeGroups).forEach(([fishName, purchases]) => {
        ...
        // Add each purchase as a separate row with fish name in the first row only
        sortedPurchases.forEach((purchase, index) => {
          ...
          // Add row with all details in the same row
          const purchaseRow = worksheet.addRow([
            index === 0 ? counter.toString() : '', // Number only for first item in group
            index === 0 ? fishName.toUpperCase() + (fishName.includes('-') ? '' : '-W') : '', // Fish name only for first item
          ...
          // Increment counter only after the first row of each fish type
          if (index === 0) {
            counter++;
          }
          ...
          purchaseRow.eachCell((cell, colNumber) => {
            ...
            cell.font = { size: 10 }; // Larger font size for better readability
        ...
        subtotalRow.eachCell((cell, colNumber) => {
          ...
          // Add light gray background to the entire subtotal row
          cell.fill = {
            type: 'pattern',
            pattern: 'solid',
          ...
          // Use larger font for subtotal row
          if (colNumber === 2) {
        ...
        // Format numbers in subtotal with commas
      ...
      // Style the grand total row - entire row with red background
      grandTotalRow.eachCell((cell, colNumber) => {
        ...
        // Add red background to the entire grand total row
        cell.fill = {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: 'FF9999' } // Deeper red background for entire row
        };
        
        // Increase font size for all cells in grand total row
        if (colNumber === 2) {
          cell.font = { bold: true, size: 10 };
          cell.alignment = { horizontal: 'right', vertical: 'middle' };
      ...
      // Special formatting for the grand total amount (bold red text)
      const totalAmountCell = grandTotalRow.getCell(8);
      totalAmountCell.font = { bold: true, size: 10, color: { argb: 'FF0000' } }; // Bold red text for total
      totalAmountCell.numFmt = '#,##0.00';
      totalAmountCell.alignment = { horizontal: 'right', vertical: 'middle' };
      totalAmountCell.value = grandTotal.toFixed(2);
      
      // Format other cells in grand total row
